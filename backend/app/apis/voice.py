...
async def handle_incoming_call(request: Request, db: Session = Depends(get_db)):
    ...
    # ------------------------------------------------------------------
    # DEPRECATION NOTICE:
    # The custom WebSocket media stream architecture has been removed.
    # We now strictly use LiveKit SIP Trunking for voice calls.
    # ------------------------------------------------------------------
    
    logger.warning("Received call on deprecated /incoming-call webhook. Rejecting in favor of SIP.")
    
    # Play a message to the caller explaining the situation (optional)
    response.say("This number is configured for SIP trunking. Please check your system configuration.")
    response.hangup()
