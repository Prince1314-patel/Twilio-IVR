...
Dependencies:
    - langchain_core
    - app.database.manager (local module)
...
# Optimization: Eliminates 10-50ms connection setup overhead per tool call
...
def get_next_closest_slots(date, requested_time, max_slots=5):
    ...
    """
    slots = _db_manager.get_available_slots(date)
...
def check_appointment_availability(date: str, time: str) -> str:
    ...
    Returns:
        str: Availability message or error message as a JSON string.
    """
    ...
    if not valid:
        ...
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": msg})
    ...
    if not valid:
        logger.warning(f"Tool: check_appointment_availability | Validation failed: {msg}")
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Cannot check availability for a past date/time."})
    valid, msg = is_within_business_hours(time)
    if not valid:
        logger.warning(f"Tool: check_appointment_availability | Validation failed: {msg}")
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Time must be within business hours (09:00 to 17:00)."})
    valid, msg = is_slot_granular(time)
    if not valid:
        logger.warning(f"Tool: check_appointment_availability | Validation failed: {msg}")
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Appointments can only be booked on the hour or half-hour (e.g., 09:00:00, 09:30:00)."})
    try:
        is_available = _db_manager.check_availability(date, time)
        if is_available:
            result = f"Time slot {time} on {date} is available."
            logger.info(f"Tool: check_appointment_availability | Result: {result}")
            return json.dumps({"success": True, "code": "AVAILABLE", "payload": result})
        else:
            ...
            if alt_slots:
                ...
                logger.info(f"Tool: check_appointment_availability | Result: {result}")
                return json.dumps({"success": False, "code": "UNAVAILABLE_WITH_ALTERNATES", "payload": result})
            else:
                result = f"Time slot {time} on {date} is not available. No available slots for this date."
                logger.info(f"Tool: check_appointment_availability | Result: {result}")
                return json.dumps({"success": False, "code": "UNAVAILABLE", "payload": result})
    except Exception as e:
        error_logger.error(f"Tool: check_appointment_availability | Exception: {str(e)}")
        return json.dumps({"success": False, "code": "ERROR", "payload": f"Error checking availability: {str(e)}"})
...
def create_appointment_in_db(name: str, mobile_number: str, appointment_type: str, 
    ...
    Returns:
        str: Success or error message as a JSON string.
    """
    ...
    if not valid:
        logger.warning(f"Tool: create_appointment_in_db | Validation failed: {msg}")
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Cannot book an appointment in the past."})
    valid, msg = is_within_business_hours(time)
    if not valid:
        logger.warning(f"Tool: create_appointment_in_db | Validation failed: {msg}")
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Time must be within business hours (09:00 to 17:00)."})
    ...
    if not _db_manager.check_availability(date, time):
        # Suggest next closest available slots
        alt_slots = get_next_closest_slots(date, time)
        if alt_slots:
            slots_str = ", ".join(alt_slots)
            ...
            return json.dumps({"success": False, "code": "UNAVAILABLE", "payload": result})
    try:
        result = _db_manager.create_appointment(name, mobile_number, appointment_type, date, time, notes)
        if result["success"]:
            msg = f"Appointment created successfully with ID: {result['appointment_id']}, Token Number: {result.get('token_number')}"
            logger.info(f"Tool: create_appointment_in_db | Result: {msg}")
            return json.dumps({"success": True, "code": "SUCCESS", "payload": msg})
        else:
            msg = f"Failed to create appointment: {result['message']}"
            logger.warning(f"Tool: create_appointment_in_db | Result: {msg}")
            return json.dumps({"success": False, "code": "CREATION_FAILED", "payload": msg})
    except Exception as e:
        error_logger.error(f"Tool: create_appointment_in_db | Exception: {str(e)}")
        return json.dumps({"success": False, "code": "ERROR", "payload": f"Error creating appointment: {str(e)}"})
...
def get_available_slots_for_date(date: str) -> str:
    ...
    try:
        ...
        if slots:
            ...
            return json.dumps({"success": True, "code": "AVAILABLE_SLOTS", "payload": result, "slots": slots})
        else:
            result = f"No available slots for {date}"
            logger.info(f"Tool: get_available_slots_for_date | Result: {result}")
            return json.dumps({"success": True, "code": "NO_SLOTS", "payload": result, "slots": []})
    except Exception as e:
        error_logger.error(f"Tool: get_available_slots_for_date | Exception: {str(e)}")
        return json.dumps({"success": False, "code": "ERROR", "payload": f"Error getting available slots: {str(e)}"})

@tool
def update_appointment_in_db(appointment_id: int, caller_mobile_number: str = None, name: str = None, mobile_number: str = None, appointment_type: str = None, date: str = None, time: str = None, status: str = None, notes: str = None) -> str:
    """
    ...
    Args:
        ...
        caller_mobile_number (str, optional): The mobile number of the user making the request, for ownership validation.
        ...
        notes (str, optional): New notes (symptoms). Should be in English.
    Returns:
        str: Success or error message as a JSON string.
    """
    log_input = {'appointment_id': appointment_id, 'caller_mobile_number': mask_mobile_number(caller_mobile_number), 'name': name, 'mobile_number': mask_mobile_number(mobile_number) if mobile_number else None, 'appointment_type': appointment_type, 'date': date, 'time': time, 'status': status, 'notes': notes}
    logger.info(f"Tool: update_appointment_in_db | Input: {log_input}")
    if not isinstance(appointment_id, int) or appointment_id <= 0:
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Invalid appointment ID."})
    update_fields = {}
    if name is not None:
        valid, msg = validate_name(name)
        if not valid:
    ...
    if not update_fields:
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "No valid fields provided to update."})
    if caller_mobile_number is not None:
        update_fields["caller_mobile_number"] = caller_mobile_number
    try:
        result = _db_manager.update_appointment(appointment_id, **update_fields)
        if result["success"]:
            msg = f"Appointment updated successfully."
            logger.info(f"Tool: update_appointment_in_db | Result: {msg}")
            return json.dumps({"success": True, "code": "SUCCESS", "payload": msg})
        else:
            # Enhanced error message for better agent understanding
            msg = f"Failed to update appointment: {result['message']}"
            if "already booked" in result['message']:
                msg += " Please check availability for a different time."
            
            logger.warning(f"Tool: update_appointment_in_db | Result: {msg}")
            return json.dumps({"success": False, "code": "UPDATE_FAILED", "payload": msg})
    except Exception as e:
        error_logger.error(f"Tool: update_appointment_in_db | Exception: {str(e)}")
        return json.dumps({"success": False, "code": "ERROR", "payload": f"Error updating appointment: {str(e)}"})

@tool
def cancel_appointment_in_db(appointment_id: int, reason: str, caller_mobile_number: str = None) -> str:
    """
    ...
    Returns:
        str: Success or error message as a JSON string.
    """
    log_input = {'appointment_id': appointment_id, 'reason': reason, 'caller_mobile_number': mask_mobile_number(caller_mobile_number)}
    logger.info(f"Tool: cancel_appointment_in_db | Input: {log_input}")
    if not isinstance(appointment_id, int) or appointment_id <= 0:
        return json.dumps({"success": False, "code": "VALIDATION_FAILED", "payload": "Invalid appointment ID."})
    try:
        result = _db_manager.cancel_appointment(appointment_id, reason=reason, caller_mobile_number=caller_mobile_number)
        if result["success"]:
            ...
            return json.dumps({"success": False, "code": "CANCELLATION_FAILED", "payload": msg})
    except Exception as e:
        error_logger.error(f"Tool: cancel_appointment_in_db | Exception: {str(e)}")
        return json.dumps({"success": False, "code": "ERROR", "payload": f"Error cancelling appointment: {str(e)}"})

@tool
def get_upcoming_appointments(mobile_number: str) -> str:
    """
    Get all upcoming appointments for a user based on their mobile number.
    
    Args:
        mobile_number (str): The user's mobile number.
        
    Returns:
        str: A JSON string containing a list of upcoming appointments.
    """
    log_input = {'mobile_number': mask_mobile_number(mobile_number)}
    logger.info(f"Tool: get_upcoming_appointments | Input: {log_input}")
    ...
    try:
        ...
        user_info = _db_manager.get_user_by_mobile_number(mobile_number)
        if not user_info:
            return json.dumps({"success": True, "code": "NO_APPOINTMENTS", "payload": []})
            
        user_id = user_info['user_id']
        
        appointments = _db_manager.get_user_appointment_history(user_id, limit=10)
        ...
        for appt in appointments:
            ...
            appt_time = datetime.datetime.strptime(appt['appointment_time'], "%H:%M:%S").time()
        ...
        json_result = json.dumps({"success": True, "code": "SUCCESS", "payload": result_data})
        logger.info(f"Tool: get_upcoming_appointments | Result: {len(result_data)} appointments found")
        return json_result

    except Exception as e:
        error_logger.error(f"Tool: get_upcoming_appointments | Exception: {str(e)}")
        return json.dumps({"success": False, "code": "ERROR", "payload": str(e)})